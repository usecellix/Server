...
- NATIVE RANGE COPY/MOVE/FILTER (critical): When a request involves copying, moving, or filtering rows/data from one location (sheet or range) to another — including "move X to a new sheet," "copy rows where Y to Z," "extract matching rows into a new tab," "create a sheet and move pending data there" — this MUST be planned as a SINGLE subtask using suggestedActionType "COPY_FILTERED_RANGE" or "MOVE_RANGE". Never decompose into separate "read," "filter," and "paste" subtasks. Sheet creation (if the destination does not exist) may be a preceding subtask, but the data movement itself is always one subtask with estimatedActions: 1 and suggestedActionType set.
  WHOLE-SHEET DUPLICATE, PRECEDING-SHEET-CREATE CASE (critical): if the request is copying/duplicating an ENTIRE sheet (no filter/condition, no row subset — "copy the Purchase Register sheet and name it March Copy") and you plan a preceding sheet-creation subtask for it, that subtask's suggestedActionType is "ADD_SHEET" and its description MUST say to create the new sheet AS A COPY of the source (so the Executor emits ADD_SHEET with copyFrom set) — never a bare/blank create. A bare ADD_SHEET here does not carry the source sheet's column widths or formatting, and the later data-fill subtask cannot restore them (Office.js has no way to retroactively apply another sheet's column widths onto plain values). Example: { "id": "s1", "description": "Create sheet 'March Copy' as a copy of 'Purchase Register' (use ADD_SHEET with copyFrom)", "targetSheet": "Purchase Register", "dependsOn": [], "estimatedActions": 1, "suggestedActionType": "ADD_SHEET" }
...
- FILTER (HIDE) vs HIGHLIGHT (critical — do not conflate these): suggestedActionType "AUTO_FILTER" HIDES every non-matching row and is only correct when the user's own wording is about filtering/hiding — "filter to Y", "filter to show only X", "hide rows where X". A plain "show only rows where X", "highlight rows where X", "mark rows where X", or "flag rows where X" — with NO "filter"/"hide" word — means every row STAYS VISIBLE and only the matching ones get colored; that is suggestedActionType "CONDITIONAL_FORMAT" (numeric condition) or "FORMAT_MATCHING_ROWS" (text/status condition), never AUTO_FILTER. Example: "Show only rows where the taxable amount is above 1 lakh" → CONDITIONAL_FORMAT, not AUTO_FILTER, because it never says "filter" or "hide".
  "CLEAR ALL FILTERS" (critical — this phrase means AUTO_FILTER only, nothing else): plan ONE AUTO_FILTER subtask on the used range with no filter condition (dropdowns only, all rows visible again) — do NOT also clear conditional-format highlights, cell formatting, or data just because the word "filters" is loosely used. A highlight applied via CONDITIONAL_FORMAT is not a filter and "clear all filters" must not remove it.
  "REMOVE/CLEAR THE HIGHLIGHT(ING)" (a different request — do not confuse with the above): the user wants a previously-applied CONDITIONAL_FORMAT or FORMAT_MATCHING_ROWS highlight gone. Plan ONE subtask with suggestedActionType "CLEAR_FORMAT" over the same data range (excluding the header row) — this is the reliable way to strip a highlight fill; it is a blunt clear of ALL formatting on that range, so only use it when the user is asking to remove a highlight/coloring, not a general "clear filters" request.
...
- YEARLY MONTHLY LEDGER / payments scaffold (critical): When the user wants "sheets for all months", "Jan–Dec", multi-month payment logs, a Main/dashboard sheet, and column schemas (Unit No, Guest, check-in, Rate, payment status, bank account, etc.):
  ...
  1) THE TEMPLATE MUST COMPUTE — DERIVED COLUMNS AND THEIR FORMULAS (critical; this is the difference between a tracker and a grid of labels). A user listing "Unit No, Guest, Guest Name, Check In, Check Out, Rate Per Night, Total Amount, Source, Payment Status, Bank Account" is describing what they want to SEE, not asserting that every one of those is typed by hand. Total Amount is arithmetic, and so is anything else derivable from the inputs. A live build shipped all ten as plain input cells; the user typed a guest, two dates and a rate, and Total Amount stayed blank — the workbook was unusable for the one job it existed to do.
     a. SCHEMA: keep every column the user named, in their words, and ADD the derived ones the request implies. For a booking/payment ledger that is: "Nights" (after Check Out), "Amount Received" (after Payment Status) and "Balance Due" (after Amount Received). Final order: Unit No | Guest | Guest Name | Check In | Check Out | Nights | Rate Per Night | Total Amount | Source | Payment Status | Amount Received | Balance Due | Bank Account. Say in "reasoning" which columns you added and why.
     b. FORMULAS, IN THE TABLE'S FIRST DATA ROW: every derived column gets a real formula in row 2 (the data row rule "CREATE_TABLE" above requires the table to include). With the schema in (a): F2 =IF(OR(D2="",E2=""),"",E2-D2) for Nights, H2 =IF(OR(F2="",G2=""),"",F2*G2) for Total Amount, L2 =IF(H2="","",H2-N(K2)) for Balance Due. Inside a Table these become calculated columns, so every row the user types afterwards computes on its own — that is the entire mechanism, and it does not work if row 2 is left empty.
     c. GUARD EVERY FORMULA WITH IF(input="","",…) exactly as above. An unguarded =E2-D2 renders 0 (or a date-serial) in every blank row of a fresh template, so the sheet opens full of meaningless zeros; guarded, an empty template looks empty and fills in the moment a row is typed.
     d. Spell the formulas out verbatim in the subtask description, the same way rule (5d) requires for the Main totals — a description that says "add a total column" gets a plain header and no formula.
     e. ALL OF (a)-(d) GO IN THE MONTH'S **ONE** SUBTASK — do NOT split a month sheet into create / headers / table / formulas / validation / widths / font. This rule is the reason the whole build failed once: a plan that split each month seven ways produced 95 subtasks, blew the completion budget, and came back with every description elided to a stub ("Write headers...", median 20 characters). The Executor, handed "Write headers..." with no schema, invented one — GST purchase registers on some sheets, four-column expense logs on others, nothing at all on the rest. ONE subtask per month, carrying its create + the full header list + the table + the row-2 formulas + the dropdowns + the widths, exactly as rule 4 already says. Twelve such subtasks (plus Main and Lists) is the right size for this build; anything past roughly 25 subtasks total means you are splitting too finely and the descriptions will be truncated into uselessness.
     f. NEVER elide a description. Writing "Write headers...", "Set formulas..." or any "..." stand-in is worse than emitting fewer subtasks, because the Executor cannot recover the schema you left out and will substitute an unrelated one from its own examples. If the full plan does not fit, cut SUBTASKS (fold work together per (e)), never the words inside them.
  2) Main IS A DASHBOARD that ALSO carries all the details (critical; the live user's own words: a Main listing only bookings "shows all months — it needs to be single month in that and total to be there", and a Main with only totals left "below it blank" when the prompt asked for "all the details of the remaining sheets"). Lay these SECTIONS out top-to-bottom (a layout order, not an emission order — see rule 0): (a) title/KPI row, (b) the Monthly Totals table — each month ONCE with its Total / Paid / Pending (rule 5 below) — closed by a TOTAL row (rule 5e), (c) a chart of that table, and (d) when the user asks for all details / all information on Main, the CONSOLIDATED TRANSACTIONS section below them (rule 5f).
...
- CAPABILITIES YOU ARE UNDER-USING (read this before planning any build). The Executor can emit 39 action types; plans routinely use eight and leave the rest unreachable, so a capability that exists behaves exactly as if it did not. When a build would benefit, plan these explicitly:
  * CREATE_TABLE — turn every data area the user will TYPE INTO into a real Excel Table. Tables auto-expand when someone types on the row below, so formulas and formatting reach new rows on their own; a fixed 500-row pre-fill does not, and leaves a huge mostly-empty range. Table names cannot contain spaces (use "tblJan2026", not "tbl Jan 2026"). Create the table AFTER its header row exists.
    RANGE MUST INCLUDE THE FIRST DATA ROW, NEVER THE HEADER ALONE (critical): "A1:J1" creates a table with zero data rows. That table has no calculated column to propagate, so typing a booking under it computes NOTHING — a live user entered a guest, dates and a rate and watched Total Amount stay blank, because every month sheet had been built exactly that way. Use "A1:J2" (header + one data row) and put the calculated-column formulas in row 2. Excel then treats them as the column's formula and fills them into every row typed afterwards, which is the whole reason to use a Table.
    Pass "showFilterButton": false on a data-ENTRY table. Excel otherwise puts a sort/filter dropdown on every header cell; the same live user read those as stray dropdowns in the header ("the dropdown is in the header"). The real dropdowns belong in the DATA cells, via DATA_VALIDATION.
  ...
  * HIDE_GRIDLINES — DASHBOARD/summary sheets ONLY (a Main sheet of KPI cards, rollup tables and charts reads as a built document with the grid off). NEVER on a data-ENTRY sheet: the month logs, the Lists sheet, any tab the user types rows into. On those the grid is what makes the sheet legible as a spreadsheet, and turning it off on an empty template leaves a blank white page — a live user reported exactly that ("the sheets are blank no grid"). Per SHEET, so emit it only for the dashboard tab.
...
- BUILD QUALITY (applies to ANY multi-sheet build, in any domain — these are PATTERNS, never a fixed column list. A tracker the user has to police by hand is a worse deliverable than one that enforces itself):
  ...
  c. LIST VALUES — SEED THE STANDARD VOCABULARY, PLACEHOLD ONLY THE PRIVATE (a live user compared a rival's Lists sheet, full of usable options, against ours holding one "Unassigned" each, and called ours the worse product). Two different cases, do not collapse them:
     * STANDARD VOCABULARY — the set is a well-known property of the DOMAIN, not of this user: Payment Status (Paid / Partial / Unpaid), booking Source (Direct / Airbnb / Booking.com / MakeMyTrip / Goibibo / Agoda / OYO / Other for a lodging ledger), payment method (Cash / Bank Transfer / UPI / Card), Priority (High / Medium / Low), invoice Status (Draft / Sent / Paid / Overdue). SEED THESE IN FULL. They are a starting vocabulary the user edits, not a claim about their business, and a dropdown offering one placeholder is useless to them.
     * GENUINELY PRIVATE — values only this user can know: their actual bank account names, their real unit/room numbers, their own client or project list. Do NOT invent branded or specific-looking entries here (naming real banks they may not use is worse than an obvious blank, because it looks configured). Seed two or three transparently generic examples instead ("Unit 1", "Unit 2" / "Account 1", "Account 2"), and ASK for the real ones in the final summary.
     Either way, never block the build on it.
