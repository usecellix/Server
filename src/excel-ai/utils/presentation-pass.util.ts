...
export function applyPresentationPass(
  ...
  for (const run of runs) {
    ...
    for (const [col, label] of run.labels) {
      // The veto runs first and wins: a text or identifier column must stay
      // unformatted even when it contains a money/count word ("Guest Name",
      // "Payment Status", "Unit No", "Invoice Number"). TASKS.md #158.
      const numberFormat = NON_NUMERIC_HEADER_PATTERN.test(label)
        ? null
        : COUNT_HEADER_PATTERN.test(label)
          ? '#,##0'
          : CURRENCY_HEADER_PATTERN.test(label)
            ? currencyFormat
            : null;
  ...
  // Per-sheet finishing: title styling, frozen header, autofit.
...
function findTitleRow(actions: SheetActionPayload[], header: HeaderRun): number | null {
  const key = header.sheetName.toLowerCase();
  const rows = new Map<number, number>();

  for (const action of actions) {
    if (action.type !== 'SET_CELL') continue;
    if (String(action.sheetName ?? '').trim().toLowerCase() !== key) continue;
    if (typeof action.row !== 'number' || action.row >= header.row) continue;
    if (typeof action.value !== 'string' || action.value.trim() === '') continue;
    rows.set(action.row, (rows.get(action.row) ?? 0) + 1);
  }
